...
const MobileCategoryList: React.FC<MobileCategoryListProps> = ({ onNavigate }) => {
        ...
        const fetchAndGroup = async () => {
            ...
            try {
                ...
                apiSubcategorias.forEach(subCat => {
                    ...
                    const childCategory: FilterCategory = { id: subCat.id, nombre: subCat.nombre };
                ...
                console.error("❌ Error al cargar y agrupar categorías en el menú móvil:", error);
            } finally {
                setIsLoading(false);
            }
        };
    ...
    if (isLoading) {
        return <div className="p-3 text-sm text-gray-500">Cargando productos...</div>;
    }
                    ...
                    <div key={category.id} className="space-y-1">
                        ...
                        <div 
                            ...
                            className={`flex items-center justify-between rounded-md transition duration-150 
                                ${isCategoryOpen ? 'bg-amber-100' : 'bg-gray-50 hover:bg-gray-100'}
                            `}
                        >
                            ...
                            <Link
                                ...
                                className={`flex-1 px-3 py-2 text-base font-bold ${isCategoryOpen ? 'text-amber-700' : 'text-gray-800'}`}
                            >
                                {category.nombre}
                            </Link>
                            ...
                            {hasSubcategories && (
                                <button
                                    ...
                                    className={`p-2 transition-colors ${isCategoryOpen ? 'text-amber-700' : 'text-gray-600 hover:text-amber-700'}`}
                                    aria-expanded={isCategoryOpen}
                                >
                                    <svg className={`h-5 w-5 transform transition-transform ${isCategoryOpen ? 'rotate-180' : 'rotate-0'}`} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                        ...
                        {isCategoryOpen && hasSubcategories && (
                            <div 
                                className="pl-4 space-y-0.5 border-l border-gray-100 ml-4 pt-1"
                            >
