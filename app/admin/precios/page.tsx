...
export default function PreciosPage() {
  ...
  const [estadoPromocionFiltro, setEstadoPromocionFiltro] =
  ...
  } = useCrudCatalog<PrecioConProducto, CreatePrecioData, UpdatePrecioData>(
    {
      ...
      loadItems: async (all, page, size, searchTerm, promocionFiltro) => {
        ...
        if (promocionFiltro && String(promocionFiltro).trim() !== "") {
          const prodsRespAll = await getProductos(
            1,
            Math.max(1000, size),
            "",
            searchTerm
          ...
          const allProductos =
            (prodsRespAll as any).data ||
            (Array.isArray(prodsRespAll) ? prodsRespAll : []);

          // Traemos precios para enriquecer
          let preciosListAll: any[] = [];
          try {
            const preciosResp = await getPrecios(1, 1000, "");
            preciosListAll = Array.isArray(preciosResp)
              ? preciosResp
              : (preciosResp as any).data || [];
          } catch (e) {
            preciosListAll = [];
          }
            ...
            if (precioObj) {
              return {
                ...
                valor_unitario: Number(
                  precioObj.valor_unitario ?? precioObj.valor ?? p.precio ?? 0
                ),
                descuento_porcentaje: Number(
                  precioObj.descuento_porcentaje ?? 0
                ),
                valor_final: Number(
                  precioObj.valor_final ??
                    Number(precioObj.valor_unitario ?? 0) *
                      (1 - Number(precioObj.descuento_porcentaje ?? 0) / 100)
                ),
                estado:
                  precioObj.estado ||
                  (Number(precioObj.descuento_porcentaje ?? 0) > 0
                    ? "En Promoción"
                    : "Normal"),
          ...
          try {
            ...
            console.debug(
              "[PreciosPage.loadItems] promocionFiltro=",
              promocionFiltro,
              "allProductos=",
              allProductos.length,
              "preciosListAll=",
              preciosListAll.length,
              "finalList=",
              finalList.length
            );
          } catch (e) {}
        ...
        try {
          const preciosResp = await getPrecios(1, 1000, "");
          preciosList = Array.isArray(preciosResp)
            ? preciosResp
            : (preciosResp as any).data || [];
        } catch (e) {
          preciosList = [];
      ...
      createItem: async (data: CreatePrecioData) => {
        console.log(
          "[PreciosPage.createItem] datos recibidos del formulario:",
          data
        );
        ...
        try {
          const created = await (
            await import("@/components/services/preciosService")
          ).createPrecio(data);
          // Si el servicio devolvió el precio creado, tratamos de obtener el producto relacionado
          const producto =
            created && (created.producto || created.productoId)
              ? await getProductoById(
                  Number(created.productoId ?? created.producto?.id)
                )
          ...
          const valor_final = Number(
            (data as any).valor_final ?? valor_unitario * (1 - descuento / 100)
          );
          return {
            id: created?.id ?? 0,
      ...
      updateItem: async (id: number, data: UpdatePrecioData) => {
        console.log(
          "[PreciosPage.updateItem] editar precio id=",
          id,
          "data=",
          data
        );
        try {
          const preciosService = await import(
            "@/components/services/preciosService"
          );
          const updated = await preciosService.updatePrecio(id, data);
          const pid = Number(
            (data as any).productoId ?? updated?.productoId ?? 0
          );
          ...
          console.error(
            "[PreciosPage.updateItem] error actualizando precio:",
            err
          );
          throw err;
        }
  ...
  const updateStats = React.useCallback(async () => {
    try {
      ...
      const totalProductos =
        (prodsResp as any).total ??
        (Array.isArray(prodsResp) ? prodsResp.length : 0);
      ...
      try {
        ...
        productosForAvg =
          (prodsAll as any).data || (Array.isArray(prodsAll) ? prodsAll : []);
      } catch (e) {
        productosForAvg = [];
      }
      const preciosNums = productosForAvg
        .map((p) => Number(p.precio ?? 0))
        .filter((v) => !isNaN(v));
      const precioPromedio =
        preciosNums.length > 0
          ? preciosNums.reduce((a, b) => a + b, 0) / preciosNums.length
          : 0;
  ...
  const handleFormSubmitWithStats = async (
    data: CreatePrecioData | UpdatePrecioData
  ) => {
    setFormError("");
    try {
      ...
      const msg =
        ...
        error?.message ||
        "Error al guardar el precio/promoción.";
      setFormError(msg);
    }
  };
  ...
  const handleImport = () => {
    alert(
      "Función de Importar Precios no implementada. [Pendiente de desarrollo]"
    );
  };
      ...
      <div className="space-y-6">
        ...
        {showModal && (
          <ModalVentana
            ...
            title={
              editingPrecio
                ? "Editar Precio/Promoción"
                : "Nuevo Precio/Promoción"
            }
          >
            <PreciosForm
